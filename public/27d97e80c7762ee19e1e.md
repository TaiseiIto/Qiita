...
tags:
  - 自作OS
private: false
updated_at: '2025-12-18T13:42:53+09:00'
...
以下の手順でliumOSのディレクトリツリー`mnt`およびそれを圧縮した`liumos.zip`を得られるようになっています．
...
1. `liumos.zip`をDockerコンテナからホストにコピー
...
その後，liumOSをクローンし，特定のコミットにチェックアウトし，ビルドスクリプト`build_on_container.sh`をコンテナ内に配置して環境構築は終わりです．
...
GitHubからビルド済みのliumOSをダウンロードできるようになりました．
